...
import { Wallet, TrendingUp, RefreshCw, Receipt } from "lucide-react";
import {
  ResponsiveContainer,
  BarChart,
  Bar,
...
export const Route = createFileRoute("/_app/financial-reconciliation")({
  component: FinanceReconPage,
});
...
function FinanceReconPage() {
  ...
  // ---------- handlers ----------
  function handleApproveClick(row: PayoutRow) {
    setApproving(row);
  }
  ...
  function handleModalApprove() {
    ...
    setPayouts(prev =>
      prev.map(p => (p.id === approving.id ? { ...p, status: "approved" } : p))
    );
    ...
    // 3. Close approve modal, open success modal
    setApproving(null);
    setSuccessInfo({
      batchId: approving.id,
      batchName: approving.batch,
      netPayout: approving.net,
      approvedAt,
    });
  }
    ...
    <>
      ...
      {successInfo && (
        <ApprovalSuccessModal
          ...
          batchName={successInfo.batchName}
          netPayout={successInfo.netPayout}
          approvedBy="Admin User"
          approvedAt={successInfo.approvedAt}
      ...
      <div className="space-y-6">
                  ...
                  <td className="px-5 py-3">
                    {p.status === "pending" && (
                      <button
                        ...
                        className="px-3 py-1.5 text-[12px] rounded-md bg-primary text-white hover:bg-primary/90 transition-colors"
                      >
                        Approve
                      </button>
                    )}
        ...
        {/* Audit Log (shown only after at least one approval) */}
        {auditLog.length > 0 && (
          <Card title="Audit Log" padded={false}>
            <table className="w-full text-[13px]">
              <thead>
                <tr className="bg-muted/40 text-muted-foreground text-left">
                  <th className="px-5 py-2.5 font-medium">Entry ID</th>
              ...
              <tbody>
                {auditLog.map(e => (
                  <tr key={e.id} className="border-t border-border hover:bg-muted/30">
                    ...
                    <td className="px-5 py-3">
                      <StatusChip tone="success" label={e.action} dot />
                    </td>
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="bg-muted/40 text-muted-foreground text-left">
                ...
                <th className="px-5 py-2.5 font-medium">Date</th>
                <th className="px-5 py-2.5 font-medium">Debit</th>
