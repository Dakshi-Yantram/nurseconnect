      ...
      <div className="px-4 py-3 grid grid-cols-2 gap-x-4 gap-y-2">
        {dateTime && (
          <div className="flex items-center gap-1.5 text-[12px] text-muted-foreground">
            ...
            <span className="truncate">{dateTime}{duration ? ` Â· ${duration}` : ""}</span>
          </div>
        )}
        {location && (
          <div className="flex items-center gap-1.5 text-[12px] text-muted-foreground">
...
export const Route = createFileRoute("/_app/partner/assignments")({
  ...
  head: () => ({ meta: [{ title: "Assignments â€” NurseConnect" }] }),
});
...
function WorkerAssignments() {
  ...
  const tryClaim = (id: string) => {
    // â† eligibility check removed â€” Accept works regardless of training status
    claim(
      {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Eligibility banner â€” still shown as info, but no longer blocks Accept */}
      ...
      {import.meta.env.DEV && (
        <details className="rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-[11px] text-blue-800">
          <summary className="cursor-pointer font-semibold">
            Debug: {unclaimedAll.length} unclaimed Â· {open.length} claimable Â· worker: {claimantId}
          </summary>
            ...
            {unclaimedAll.length === 0
              ...
              : unclaimedAll.map(r => (
                  <div key={r.id} className="font-mono">
                    #{r.id} state=<strong>{r.state}</strong> claimantId={String((r as any).claimantId ?? "â€”")}
                  </div>
                ))}
          </div>
        </details>
