...
import {
  GraduationCap, ShieldCheck, AlertTriangle, Clock,
  BookOpen, CheckCircle2, XCircle, RefreshCw, ChevronRight, Sparkles,
} from "lucide-react";

function WorkerTrainingError({ error, reset }: { error: Error; reset: () => void }) {
...
export const Route = createFileRoute("/_app/partner/training")({
  ...
  head: () => ({ meta: [{ title: "Training â€” NurseConnect" }] }),
...
> = {
  ...
  submitted: { bg: "bg-sky-50",    text: "text-sky-700",     border: "border-sky-200",     dot: "bg-sky-500",     label: "Awaiting Review" },
  pending:  { bg: "bg-orange-50",  text: "text-orange-700",  border: "border-orange-200",  dot: "bg-orange-400",  label: "Not Started" },
  rejected: { bg: "bg-rose-50",    text: "text-rose-700",    border: "border-rose-200",    dot: "bg-rose-500",    label: "Resubmit Required" },
};

function StatusBadge({ status }: { status: CompetencyStatus }) {
  const cfg = STATUS_CONFIG[status];
  return (
    <span className={`inline-flex items-center gap-1.5 text-[11px] font-medium px-2.5 py-[3px] rounded-full border ${cfg.bg} ${cfg.text} ${cfg.border}`}>
...
function EligibilitySection({ eligibility }: { eligibility: ReturnType<typeof useWorkerEligibility> }) {
  const pct = eligibility.total > 0 ? Math.round((eligibility.approved / eligibility.total) * 100) : 0;
  const isComplete = eligibility.eligible;

  return (
    <div
      className={`rounded-2xl border p-5 ${isComplete ? "bg-emerald-50 border-emerald-200" : "bg-white border-border"}`}
      style={{ boxShadow: "0 1px 8px 0 rgba(0,0,0,0.05)" }}
    >
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className={`h-8 w-8 rounded-xl grid place-items-center ${isComplete ? "bg-emerald-100 text-emerald-600" : "bg-orange-100 text-orange-500"}`}>
            {isComplete ? <ShieldCheck className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4" />}
          </div>
          <div>
        ...
        <div className={`text-[22px] font-bold tabular-nums ${isComplete ? "text-emerald-600" : "text-orange-500"}`}>
          {pct}%
        </div>
      </div>
      ...
      <div className="h-2 w-full rounded-full bg-gray-200 overflow-hidden mb-2">
        <div
          className={`h-full rounded-full transition-all duration-500 ${isComplete ? "bg-emerald-500" : "bg-orange-400"}`}
          style={{ width: `${pct}%` }}
        />
      </div>
      ...
      {!isComplete && eligibility.blockers.length > 0 && (
        <div className="space-y-1.5 mt-3 pt-3 border-t border-border">
          <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2">Missing</div>
          {eligibility.blockers.map((b) => (
            <div key={b.courseId} className="flex items-center justify-between gap-2 rounded-xl bg-white border border-border px-3 py-2">
              <span className="text-[12px] font-medium text-foreground truncate">{b.title}</span>
...
function CourseCard({
  ...
  enrollment: ReturnType<typeof useWorkerEnrollments>[number];
  onSubmit: () => void;
}) {
  const course = getCourse(enrollment.courseId);
  if (!course) return null;

  const canSubmit = enrollment.status === "pending" || enrollment.status === "rejected";
      ...
      <div className="px-4 pt-4 pb-0">
        <div className="flex items-start gap-2.5 mb-2.5">
          ...
          <div className="min-w-0 flex-1 pt-0.5">
            <div className="flex flex-wrap items-center gap-1.5">
              <span className="text-[14px] font-medium text-foreground leading-snug">{course.title}</span>
              {course.required && (
        ...
        <p className="text-[12px] text-muted-foreground leading-[1.55] mb-2.5">
          ...
          {enrollment.notes && enrollment.status === "rejected" && (
            <span className="block mt-1 text-[11.5px] text-rose-600">Reviewer: {enrollment.notes}</span>
          )}
        </p>
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap pb-3.5">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1 text-[11.5px] text-muted-foreground">
              <Clock className="h-[13px] w-[13px]" />{course.duration}
            </span>
    ...
    <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
      {tabs.map((t) => (
        ...
        >
          {t.label}
          {counts[t.key] > 0 && (
            <span className={`ml-1 text-[10px] px-1 rounded-full ${active === t.key ? "bg-gray-100 text-muted-foreground" : "bg-gray-200 text-muted-foreground"}`}>
              {counts[t.key]}
            </span>
          )}
        </button>
...
function WorkerTraining() {
  ...
  const counts: Record<FilterTab, number> = {
    ...
    in_review: enrollments.filter(e => e.status === "submitted").length,
  ...
  // Auto-approve: submit first, then immediately approve
  const handleComplete = (courseId: string) => {
    submit(workerId, workerName, courseId);
    // enrollment id format is "workerId:courseId"
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-muted-foreground mt-0.5">
            {enrollments.length} course{enrollments.length !== 1 ? "s" : ""} assigned Â·{" "}
        ...
        {filtered.length === 0 ? (
          <div className="rounded-2xl border border-border bg-white flex flex-col items-center justify-center py-14 px-6 text-center"
            ...
            <p className="text-[12.5px] text-muted-foreground mt-1 max-w-[240px]">
              {activeFilter === "all" ? "Your administrator will assign training modules here." : "Try a different filter."}
            </p>
          </div>
        ) : (
          <div className="space-y-3">
