...
export const Route = createFileRoute("/_app/partner/documentation")({
  ...
  head: () => ({ meta: [{ title: "Documentation â€” NurseConnect" }] }),
});
...
function WorkerDocumentation() {
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      <div className="border-b border-border bg-background px-6 py-4">
        <div className="mx-auto flex max-w-2xl flex-wrap items-center gap-3">
          ...
          >
            {rows.map(r => (
              <option key={r.id} value={r.id}>
                #{r.id} Â· {bookingPatientName(r) ?? "â€”"} Â· {bookingService(r) ?? ""}
              </option>
            ))}
          </select>
          ...
          {visit && (
            ...
            >
              Open visit workspace â†’
            </Link>
          )}
        </div>
      </div>

      {/* â”€â”€ Clinical workflow panel â”€â”€ */}
      ...
      {/* Visit activity log â€” rendered OUTSIDE the space-y-5 container to prevent stacking/overlap issues */}
      {visit && (
