...
export function deriveExecutionStage(rec: EntityRecord | undefined | null): ExecutionStage {
  ...
  switch (rec.state) {
    ...
    case "active":    return fallback(d.consentAccepted ? "ready_for_visit" : "check_in_pending");
    case "in_progress": {
      if (!d.checklistComplete)     return fallback("checklist_pending");
      if (!d.documentationComplete) return fallback("documentation_pending");
