...
class CompetencyStore {
  ...
  private notify() { this.version++; this.workerCache.clear(); this.reviewCache = null; this.listeners.forEach(l => l()); }
...
export function useReviewerCompetencyLifecycle(): ReviewerCompetencyLifecycle {
  return useStore(s => {
    ...
    let awaiting = 0, resubmissions = 0, approvedRecent = 0, rejectedRecent = 0;
    let lastDecisionAt: string | undefined;
    for (const e of all) {
      if (e.status === "submitted") {
        awaiting++;
        if ((e.submissionCount ?? 0) > 1 || (e.priorDecisions?.length ?? 0) > 0) resubmissions++;
      }
      if (e.status === "approved" || e.status === "rejected") {
        ...
        if (!Number.isNaN(ts) && ts >= since) {
          if (e.status === "approved") approvedRecent++; else rejectedRecent++;
        }
      }
    }
    return {
      awaiting, resubmissions,
      approvedRecent, rejectedRecent,
